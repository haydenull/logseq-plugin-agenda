...
export const moveBlockToSpecificBlock = async (
  ...
  if (!targetBlock)
    targetBlock = await logseq.Editor.insertBlock(targetPageName, targetBlockContent, {
      before: true,
      isPageBlock: true,
    })
  if (targetBlock) {
...
export const createBlockToSpecificBlock = async (
  targetPageName: string,
  targetBlockContent: string,
  blockContent: string,
  ...
  return await logseq.Editor.insertBlock(targetBlock!.uuid, blockContent, {
    isPageBlock: false,
...
export const extractBlockContentToHtml = async (block: BlockEntity, depth = 1): Promise<string> => {
  ...
  if (children && children?.length > 0) {
    const childrenContent = (
      await Promise.all(children.map(async (child) => extractBlockContentToHtml(child as BlockEntity, depth + 1)))
    ).join('')
